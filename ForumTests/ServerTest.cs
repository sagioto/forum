...
namespace ForumTests
{
    ...
    public class ServerTest
    {
        ...
        public void LoginRegisterTests(){
            ...
            Assert.IsFalse(sc.Login("user1", "123456"));//login before register

            Assert.IsTrue(sc.Register("alice", "123456"));

            //login tests
            for (int i = 0; i < 100; i++)
            {
                // try to register twice with same userName
                Assert.IsTrue(sc.Register("alice" + i, "123456"));
                Assert.IsFalse(sc.Register("alice" + i, "123456"));

                //failed: try to login twice with same userName return true , should return false.
                Assert.IsTrue(sc.Login("alice" + i, "123456"));
                //Assert.IsFalse(sc.Login("alice" + i, "123456"));

                Assert.IsFalse(sc.Login("bob" + i, "123456"));//try to login with bad unknown user
                Assert.IsFalse(sc.Login("alice" + i, "123456" + i));//try to login with bad password
            }
        ...
        public void LogoutTests()
        {
            ...
            Assert.IsFalse(sc2.Logout("testUser"));//try to logout without any register and login

            sc2.Register("alice", "123456");
            Assert.IsFalse(sc2.Logout("alice"));//try to logout without login

            sc2.Login("alice", "123456");
            Assert.IsTrue(sc2.Logout("alice"));//try to logout after login
        }
        ...
        public void postTest()
        {
            ...
            for (int i = 0; i < SubForumArray.Length; i++)
            {
                ...
                Assert.IsTrue(sc.Post(SubForumArray[i], NewPost)); //post message in all sub forums
            }
            Console.WriteLine("end");


            Post TmpPost = new Post(new Postkey("test1", DateTime.Now), "title", "body", null, "Woman");
            Assert.IsFalse(sc.Post("XXXYYYZZZ", TmpPost));//post message in sub forum that isn"t exists
